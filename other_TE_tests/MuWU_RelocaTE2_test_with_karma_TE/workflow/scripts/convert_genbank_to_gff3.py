...
"""
modified 07.06.2021:
-> only interested in gene level output; warnings for all other features not important
-> implemented "fixes" for the program to finish its run as per this discussion and not abort when encountering RNA uncertainties - https://github.com/jorvis/biocode/issues/60
-> GenBank entries are not very consistent it seems; ensemblPlants refers to most genes under 'locus_tag';
   whereas on main ensembl 'gene' is used;
-> I added try handlers to first check for 'gene' key and when that isn't found 'locus_tag';
   should both be not part of a GenBank gene element the feature is skipped
-> this handles all cases I have encountered so far


Author: Tyll Stöcker (tyll.stoecker@gmail.com)
"""
...
import annotation, things, utils
...
#                raise Exception("ERROR: unstranded feature encountered: {0}".format(feat))
            ...
            if feat.type == 'gene':
                ...
                try:
                    locus_tag = feat.qualifiers['gene'][0]
                except Exception:
                    try:
